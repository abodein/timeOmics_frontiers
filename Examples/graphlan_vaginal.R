library(tidyverse)
library(mixOmics)

# vag.spca <- read_csv("~/Documents/timeomics_analysis/CS_microbiome/Milk/Final/Vaginal_spca.csv") %>%
#     dplyr::select(molecule, cluster)
# vag.pca <- read_csv("~/Documents/timeomics_analysis/CS_microbiome/Milk/Final/vaginal_pca_all.csv") %>% rename(taxo = X4)
#
# vag.pca <- vag.pca %>% mutate(taxo = taxo %>% str_remove_all("D_.__") %>% str_replace("sp\\.", "sp") %>% str_replace_all(";","\\."))  %>%
#     arrange(taxo) %>%
#     mutate(cluster = factor(cluster, levels = c(1,-1,2,-2))) %>%
#     mutate(selected = molecule %in% csec.spca$molecule) %>% na.omit() %>%
#     mutate(taxo = str_replace(taxo, "Clostridium sensu stricto .", "Clostridium") %>%
#                str_replace("Clostridiaceae 1", "Clostridiaceae") %>%
#                str_replace(" Clostridium", "Clostridium") %>%
#                str_replace("Clostridium.\\.", "Clostridium."))
#
# vag.pca <- vag.pca %>% mutate(taxoo = str_count(taxo,"\\.")) %>%
#     mutate(taxo = paste0(taxo,str_dup(".unknown", 6-taxoo)))
#
# vag.pca <- vag.pca %>%
#     mutate(L1 = str_split(taxo, "\\.") %>% map_chr(~.x[1])) %>%
#     mutate(L2 = str_split(taxo, "\\.") %>% map_chr(~.x[2])) %>%
#     mutate(L3 = str_split(taxo, "\\.") %>% map_chr(~.x[3])) %>%
#     mutate(L4 = str_split(taxo, "\\.") %>% map_chr(~.x[4])) %>%
#     mutate(L5 = str_split(taxo, "\\.") %>% map_chr(~.x[5])) %>%
#     mutate(L6 = str_split(taxo, "\\.") %>% map_chr(~.x[6]))
#
# #vag.pca %>% write_tsv("~/ls3x/graphlan/vaginal/vag_pca.tsv")
# vag.pca %>% save(file = "~/ls3x/graphlan/vaginal/vag_pca.Rdata")


load("../Data/vag_pca.Rdata")
#vag.pca %>% dplyr::select(taxo) %>% write_tsv("~/ls3x/graphlan/vaginal/tree.txt", col_names = F)


# annotation L2
vag.pca %>% mutate(L1_L2 = paste(L1,L2, sep = ".")) %>% dplyr::select(L1_L2) %>% unique %>%
    filter(!str_detect(L1_L2, "unknown$")) %>%
    mutate(option = "annotation") %>%
    mutate(annot = str_remove(L1_L2, ".*\\.")) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_L2.tsv", col_names = F)

# annotation bacground_color L2
vag.pca %>% mutate(L1_L2 = paste(L1,L2, sep = ".")) %>% dplyr::select(L1_L2) %>% unique %>%
    filter(!str_detect(L1_L2, "unknown$")) %>%
    mutate(option = "annotation_background_color") %>%
    mutate(color = "#E8E8E8") %>%
    write_tsv("~/ls3x/graphlan/csection/annot_bacground_color_L2.tsv", col_names = F)

# L3
vag.pca %>% mutate(L1_L3 = paste(L1,L2,L3, sep = ".")) %>% dplyr::select(L1_L3) %>% unique %>%
    filter(!str_detect(L1_L3, "unknown$")) %>%
    mutate(option = "annotation") %>%
    mutate(annot = str_remove(L1_L3, ".*\\.")) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_L3.tsv", col_names = F)
vag.pca %>% mutate(L1_L3 = paste(L1,L2,L3, sep = ".")) %>% dplyr::select(L1_L3) %>% unique %>%
    filter(!str_detect(L1_L3, "unknown$")) %>%
    mutate(option = "annotation_background_color") %>%
    mutate(color = "#DADADA") %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_bacground_color_L3.tsv", col_names = F)

#L4
vag.pca %>% mutate(L1_L4 = paste(L1,L2,L3,L4, sep = ".")) %>% dplyr::select(L1_L4) %>% unique %>%
    filter(!str_detect(L1_L4, "unknown$")) %>%
    mutate(option = "annotation") %>%
    mutate(annot = str_remove(L1_L4, ".*\\.")) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_L4.tsv", col_names = F)
vag.pca %>% mutate(L1_L4 = paste(L1,L2,L3,L4, sep = ".")) %>% dplyr::select(L1_L4) %>% unique %>%
    filter(!str_detect(L1_L4, "unknown$")) %>%
    mutate(option = "annotation_background_color") %>%
    mutate(color = "#CCCCCC") %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_bacground_color_L4.tsv", col_names = F)

#L5
vag.pca %>% mutate(L1_L5 = paste(L1,L2,L3,L4,L5, sep = ".")) %>% dplyr::select(L1_L5) %>% unique %>%
    filter(!str_detect(L1_L5, "unknown$")) %>%
    mutate(option = "annotation") %>%
    mutate(annot = str_remove(L1_L5, ".*\\.")) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_L5.tsv", col_names = F)
vag.pca %>% mutate(L1_L5 = paste(L1,L2,L3,L4,L5, sep = ".")) %>% dplyr::select(L1_L5) %>% unique %>%
    filter(!str_detect(L1_L5, "unknown$")) %>%
    mutate(option = "annotation_background_color") %>%
    mutate(color = "#BCBCBC") %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_bacground_color_L5.tsv", col_names = F)

#L6
vag.pca %>% mutate(L1_L6 = paste(L1,L2,L3,L4,L5,L6, sep = ".")) %>% dplyr::select(L1_L6) %>% unique %>%
    filter(!str_detect(L1_L6, "unknown$")) %>%
    mutate(option = "annotation") %>%
    mutate(annot = str_remove(L1_L6, ".*\\.")) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_L6.tsv", col_names = F)
vag.pca %>% mutate(L1_L6 = paste(L1,L2,L3,L4,L5,L6, sep = ".")) %>% dplyr::select(L1_L6) %>% unique %>%
    filter(!str_detect(L1_L6, "unknown$")) %>%
    mutate(option = "annotation_background_color") %>%
    mutate(color = "#B0B0B0") %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_bacground_color_L6.tsv", col_names = F)

#L7
vag.pca %>% dplyr::select(taxo) %>% unique %>%
    filter(!str_detect(taxo, "unknown$")) %>%
    mutate(L7 = taxo %>% str_remove(".*\\.")) %>%
    mutate(L7b = L7 %>% str_split(" ") %>% map_chr(~.x[2])) %>%
    mutate(value = ifelse(!is.na(L7b), paste0(str_extract(L7,"^."), ". ", L7b), L7)) %>%
    mutate(option = "annotation") %>%
    dplyr::select(taxo, option, value) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_L7.tsv", col_names = F)

#selected
vag.pca %>% filter(selected) %>%
    dplyr::select(taxo) %>% mutate(option = "clade_marker_shape") %>%
    mutate(value = "*") %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_selected.tsv", col_names = F)

vag.pca %>% filter(selected) %>%
    dplyr::select(taxo) %>% mutate(option = "clade_marker_size") %>%
    mutate(value = "400") %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_selected_size.tsv", col_names = F)

# shape
vag.pca %>% mutate(color = color.mixo(cluster)) %>%
    mutate(option = "clade_marker_color") %>%
    dplyr::select(taxo, option, color) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_shape_color.tsv", col_names = F)

vag.pca %>% mutate(color = color.mixo(cluster)) %>%
    mutate(option = "annotation_background_color") %>%
    dplyr::select(taxo, option, color) %>%
    write_tsv("~/ls3x/graphlan/vaginal/annot_background_color.tsv", col_names = F)
