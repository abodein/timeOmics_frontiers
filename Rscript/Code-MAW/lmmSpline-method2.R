...
lmmSpline <- function(data, time, sampleID, timePredict, deri, basis, knots,keepModels, numCores){
  ...
  clusterExport(cl, list('data','lm','try','class','unique','anova','drop.levels','pdDiag','pdIdent','time','sampleID', 'timePredict', 'melt','dcast','predict','derivLme',
                         'knots','derivLmeCubic','lme','keepModels','basis','data','other.reshape'),envir=environment())

  # from other S3 function from jasmin
  ##clusterExport(cl, list('data.g','lm','try','class','seq','time.g','sampleID.g','data.frame','timePredict','pred.df','dataM','unique','pdIdent','predict','lme','basis','lmeControl'),envir=environment())
